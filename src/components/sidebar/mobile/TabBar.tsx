...
const TabBar = () => {
  const { items, isActive } = useSidebarNav();
        ...
        {primary.map((entry) => {
          ...
          return (
            <Link
              ...
              href={entry.path}
              className={clsx(
                "flex-1 flex flex-col items-center gap-0.5 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] text-[10px] font-semibold",
                active ? "text-primary" : "text-[#b7bcc4]"
              )}
            >
