...
type HelpContextProps = {
  id: string;
  // "pill" is the orange chip used next to a control. "link" is the compact
  // header's "▸ ¿Qué es…?" — same popover, less chrome, because the header
  // itself is trying to stay under ~72px.
  variant?: "pill" | "link";
};

const HelpContext = ({ id, variant = "pill" }: HelpContextProps) => {
  ...
  const { refs, floatingStyles, context } = useFloating({
    ...
    middleware: [offset(isLink ? 8 : -20), flip(), shift()],
  ...
  return (
    <>
      {isLink ? (
        <button
          type="button"
          className="text-body text-primary font-semibold inline-flex items-center gap-1 hover:underline"
